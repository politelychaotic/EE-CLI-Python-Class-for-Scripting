...
class RunCLI:
    ...
    def __init__(self, account_id: str):
        ...
        self.out_dir = self.working_directory / "output"
        self.account_dir = self.out_dir

        try:
            #self.out_dir.mkdir(exist_ok=False)
            self.out_dir = Path(str(self.out_dir) + "_" + self.account_id)
    ...
    def check_if_logged_in(self) -> bool:
        logged_in = False
        result = self.run([CLI, "account", "list"])

        if result == "error: token expired please login again to continue":
            logged_in = True
        elif result == "error: you are already logged in to the account":
            logged_in = False
    ...
    def login(self, username: str, password: str):
        ...
        if True:
            print(f"[+] Authenticating: {CLI} auth login --username {username} --password {password}")

            try:
                subprocess.run([CLI, "auth", "login", "--username", username, "--password", password])

            except Exception as e:
                print(f"[!!] An error occurred: {e}")
        
        if False:
            print(f"[!!] Already logged in as {self.current_account}")
    ...
    def update_cameras_by_esn(self, camfile, setting, option):
        ...
        with open(camfile, "r", encoding="utf-8") as file:
            ...
            for camera in cameras:
                ...
                elif status.stdout.split()[-1] == option:
        ...
        if pass_list:
            for i in pass_list:
                with open(passed, "w", newline="\n", encoding="utf-8") as pwrite:
                    pass
                pwrite.close()

        if failed_list:
            for i in failed_list:
                with open(failed, "w", newline="\n", encoding="utf-8") as fwrite:
                    pass
                fwrite.close()
    ...
    def get_all_camera_settings_by_esn(self, file: str, setting: str, ret: int, save: Optional[bool]=True) -> list | tuple[list,list] | None:
        ...
        :ret: returns a list: 0 -> not_set_cams, 1 -> set_cams, 2 -> not_set_cams, set_cams
        """

        not_set_cams = []
        set_cams = []

        not_set_file = self.account_dir / "not_set_cams.txt"
        set_file = self.account_dir / "set_cams.txt"
        ...
        with open(file, "r", encoding="utf-8") as reader:
            ...
            for camera in cameras:
                camera = camera.strip()
                print(f"[+] Getting camera settings")
                ...
                if setting_value in check:
                    ...
                    print("[!!] Issue figuring out value")
